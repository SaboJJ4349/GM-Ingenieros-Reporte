...
def load_and_normalize_json(file_path: str) -> pd.DataFrame:
    """
    Carga un archivo JSON desde la ruta especificada, procesa su estructura anidada
    de tareas y subtareas, y lo convierte en un DataFrame de Pandas limpio.
    """
                    ...
                    for estado, tareas in task_container.items():
                        for tarea in tareas:
                            ...
                            for subtask in subtasks:
                                ...
                                processed_subtask['id'] = f"sub_{tarea.get('id')}_{subtask.get('nombre')[:10]}"
